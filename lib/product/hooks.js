...
export function useProductPages({ creatorId, limit = 10 } = {}) {
  try {
    const { data, error, size, ...props } = useSWRInfinite(
      (index, previousPageData) => {
        // reached the end
        if (previousPageData && previousPageData.product.length === 0)
          return null;
        ...
        if (index !== 0) {
          ...
          const before = new Date(
            new Date(
              previousPageData.product[
                previousPageData.product.length - 1
              ].createdAt
            ).getTime()
          );
        ...
        return `/api/product?${searchParams.toString()}`;
      },
      fetcher,
      {
